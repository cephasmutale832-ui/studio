...
const validatePaymentPrompt = ai.definePrompt({
  ...
  prompt: `You are an expert payment validation assistant for a service called Mango SmartLearning. A student has uploaded the following transaction details: {{{transactionDetails}}}. Determine if this represents a valid payment for accessing educational materials. To be considered valid, the transaction details must explicitly mention 'money sent to Cephas Mutale' AND 'amount ZMW 70'. The isValidPayment field should be true if both keywords are present, and false otherwise. The validationResult field should contain a detailed explanation of your reasoning. Be brief.`,
});
